...
```bash
givo add @treecombinator/sdk-server-analytics
```
...
- `props` — string or number properties (`Record<string, string | number>`). Numbers become metrics (Analytics Engine `doubles`); strings become `key=value` labels (`blobs`). Keys are iterated sorted, so the same event shape always lands numbers in the same double columns, and a `doubles=<key,key,...>` blob written right after the name records which key each column holds.
...
- `track` is fire-and-forget; Analytics Engine's per-point limits (20 blobs, 20 doubles, ~5KB of blob data, 96-byte index) are enforced by capping/truncating the write instead of erroring. A missing or wrong dataset binding still throws from `writeDataPoint` itself.
